 ...
 * Works only while no user exists, and only with the deployment's CRON_SECRET.
 */
export async function createFirstOwnerInvite(input: { email: string; name: string; secret: string }): Promise<SetupResult> {
  if (!secretsMatch(input.secret, env().CRON_SECRET)) return { ok: false, error: "That setup key is not correct." };
