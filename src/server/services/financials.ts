...
import { MoneyError } from "@/core/money";
...
export interface ProjectMoney {
  ...
  headline: Headline;
  /** The amounts were out of range and couldn't be totalled. */
  broken: boolean;
}
...
export async function projectMoney(conn: DbOrTx, projectIds: string[]): Promise<Map<string, ProjectMoney>> {
  ...
  const [heads, lines, commitments, invoices, changes, units] = await Promise.all([
    ...
    conn.select({ projectId: schema.commitment.projectId, id: schema.commitment.id, budgetLineId: schema.commitment.budgetLineId, amountCents: schema.commitment.amountCents, status: schema.commitment.status }).from(schema.commitment).where(inArray(schema.commitment.projectId, projectIds)),
    ...
    conn.select({ projectId: schema.changeOrder.projectId, budgetLineId: schema.changeOrder.budgetLineId, commitmentId: schema.changeOrder.commitmentId, amountCents: schema.changeOrder.amountCents, status: schema.changeOrder.status }).from(schema.changeOrder).where(inArray(schema.changeOrder.projectId, projectIds)),
  ...
  for (const id of projectIds) {
    ...
    const typed = { useBudgetDetail: h?.useBudgetDetail ?? false, useSalesDetail: h?.useSalesDetail ?? false, purchasePriceCents: h?.purchasePriceCents ?? null, totalBudgetCents: h?.totalBudgetCents ?? null, projectedSelloutCents: h?.projectedSelloutCents ?? null, loanAmountCents: h?.loanAmountCents ?? null };
    ...
    try {
      ...
      const budget = totals.length ? totalOf(totals) : null;
      const u = units.filter((x) => x.projectId === id);
      const sales = u.length ? salesSummary(u) : null;
      out.set(id, { lines: l, totals, budget, sales, headline: headline(typed, budget, sales), broken: false });
    } catch (e) {
      // Totals out of range (MoneyError): this project shows dashes; nothing else breaks.
      if (!(e instanceof MoneyError)) throw e;
      out.set(id, { lines: l, totals: [], budget: null, sales: null, headline: headline({ ...typed, totalBudgetCents: null, projectedSelloutCents: null }, null, null), broken: true });
    }
  }
  return out;
}
