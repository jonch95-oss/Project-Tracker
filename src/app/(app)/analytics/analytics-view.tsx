...
const $sf = (c: number | null) =>
  c == null ? "—" : `${formatMoney(c, { whole: true })}/sf`;
const $ = (c: number) => formatMoney(c, { whole: true });
const typeLabel = (t: string) => PROJECT_TYPE_LABEL[t as ProjectTypeKey] ?? t;
const n1 = (x: number | null) =>
  x == null ? "—" : x.toLocaleString("en-US", { maximumFractionDigits: 1 });
/** Turnaround in days; same-day turnarounds read as "under a day", not "0 days". */
const dayText = (x: number | null) =>
  x == null ? "—" : x < 1 ? "Under a day" : `${n1(x)} day${x === 1 ? "" : "s"}`;
  ...
  return (
    <>
      <PageHeader
        eyebrow="Owner"
        title="Analytics"
        description="How long phases really take, what projects cost per square foot, where budgets move, where work stalls, and how fast RFIs and invoices turn around."
      />
      {q.isPending ? (
...
function Turnaround({ data }: { data: Data }) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      <Tile
        label="Average time to answer an RFI"
        value={dayText(t.rfi.avgDays)}
        note={
          t.rfi.answered
            ? `median ${dayText(t.rfi.medianDays).toLowerCase()} · ${t.rfi.answered} answered`
            : "No RFIs answered yet"
        }
      />
      <Tile
        label="Average time to approve an invoice"
        value={dayText(t.invoice.avgDays)}
        note={
          t.invoice.approved
            ? `median ${dayText(t.invoice.medianDays).toLowerCase()} · ${t.invoice.approved} approved`
            : "No invoices approved yet"
        }
      />
    </div>
  );
            ...
            <td className="py-1.5">
              <div className="flex items-center gap-3">
                <div className="h-3 flex-1">
                  <div
                    className="h-3 rounded-r-[4px] bg-accent transition-opacity group-hover:opacity-80"
...
function Durations({ data }: { data: Data }) {
  return (
    <Panel
      ...
      description="Median actual days from a phase's start to its finish, by project type (finished phases only)."
    >
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          {data.durations.map((d) => (
            <section key={d.type}>
              ...
              <Bars
                caption={`Median days per phase, ${typeLabel(d.type)}`}
                rows={d.phases.map((p) => ({
                  label: p.name,
                  value: p.medianDays,
                  tip: `${p.name}: median ${p.medianDays} days across ${p.n} project${p.n === 1 ? "" : "s"}`,
                }))}
                format={(v) => `${n1(v)} d`}
              />
            </section>
...
function Costs({ data }: { data: Data }) {
  return (
    <Panel
      ...
      description="Forecast cost (the larger of budget, committed and invoiced) and actual cost (invoices approved or paid), per gross and sellable sf. Benchmarks are the median by type."
    >
      {data.costs.length === 0 ? (
        <p className="text-[13px] text-muted">
          Add budgets and square footage to projects to see these.
        </p>
      ) : (
              ...
              <thead className="text-left text-[12px] text-muted">
                <tr>
                  ...
                  <th className="py-2 text-right font-medium">
                    Hard / GSF forecast · actual
                  </th>
                  <th className="py-2 text-right font-medium">
                    Total / GSF forecast · actual
                  </th>
                  <th className="py-2 text-right font-medium">
                    Total / SSF forecast · actual
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {data.costs.map((c) => (
                  <tr key={c.projectId}>
                    ...
                    <td className="py-2 text-right">
                      {$sf(c.hardPerGsf.forecast)} ·{" "}
                      <span className="text-muted">
                        {$sf(c.hardPerGsf.actual)}
                      </span>
                    </td>
                    <td className="py-2 text-right">
                      {$sf(c.totalPerGsf.forecast)} ·{" "}
                      <span className="text-muted">
                        {$sf(c.totalPerGsf.actual)}
                      </span>
                    </td>
                    <td className="py-2 text-right">
                      {$sf(c.totalPerSsf.forecast)} ·{" "}
                      <span className="text-muted">
                        {$sf(c.totalPerSsf.actual)}
                      </span>
                    </td>
                  </tr>
                ))}
...
function Variance({ data }: { data: Data }) {
  return (
    <Panel
      ...
      description="Across every project: the original budget, after approved changes, and what it's forecast to cost."
    >
...
function StallTable({
  ...
  rows: Data["stalls"]["byPhase"];
  caption: string;
  first: string;
}) {
  if (rows.length === 0)
    return <p className="text-[13px] text-muted">Nothing to show yet.</p>;
  return (
    <div className="overflow-x-auto">
          ...
          {rows.slice(0, 12).map((r) => (
            <tr key={r.key}>
              ...
              <td
                className={cn(
                  "py-2 text-right",
                  r.overdueNow > 0 && "text-blocked-text",
                )}
              >
                {r.overdueNow}
              </td>
...
function Stalls({ data }: { data: Data }) {
  return (
    <Panel
      title="Where tasks stall"
      description="What's overdue or stuck waiting now, and how late finished work ran, by phase and by person."
    >
      <div className="grid gap-8 xl:grid-cols-2">
        ...
        <section>
          ...
          <StallTable
            rows={data.stalls.byPerson}
            caption="Stalls by person"
            first="Person"
          />
        </section>
      </div>
...
function TemplateDurations() {
  ...
  const apply = useMutation(
    trpc.analytics.applyDurations.mutationOptions({
      onSuccess: async (r) => {
        toast(
          "success",
          `Template updated (version ${r.version}): ${r.changed} duration${r.changed === 1 ? "" : "s"}`,
        );
        setPicked({});
        await qc.invalidateQueries({
          queryKey: trpc.analytics.durationSuggestions.queryKey(),
        });
      },
  ...
  return (
    <Panel
      title="Update template durations from actuals"
      description="Where a template task has taken noticeably longer or shorter on three or more projects, here's the median it actually took from its phase's start. Pick the ones to adopt; new projects use them."
    >
      {q.isPending ? (
        ...
        <EmptyState
          ...
          body="Suggestions appear once a template's tasks have been done on three or more projects."
        />
      ) : (
        <div className="flex flex-col gap-8">
          {q.data.map((t) => {
            ...
            const toggle = (k: string) =>
              setPicked((p) => ({
                ...p,
                [t.templateId]: new Set(
                  sel.has(k) ? [...sel].filter((x) => x !== k) : [...sel, k],
                ),
              }));
            return (
              <section key={t.templateId}>
                <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <Button
                    ...
                    loading={
                      apply.isPending &&
                      apply.variables?.templateId === t.templateId
                    }
                    onClick={() =>
                      apply.mutate({
                        templateId: t.templateId,
                        version: t.version,
                        changes: t.suggestions
                          .filter((s) => sel.has(s.templateKey))
                          .map((s) => ({
                  ...
                  {t.suggestions.map((s) => (
                    <li key={s.templateKey}>
                      <label className="flex cursor-pointer items-center gap-3 px-4 py-3">
                        <input
                          type="checkbox"
                          checked={sel.has(s.templateKey)}
                          onChange={() => toggle(s.templateKey)}
