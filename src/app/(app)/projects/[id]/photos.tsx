...
import { compressPhoto, PhotoError, photoUrl, putObject } from "@/lib/photo-upload";
...
export function PhotoGallery({ projectId, heroPhotoId, pinnedHeroId, canUpload, canManage, viewerId }: Props) {
  const trpc = useTRPC();
  ...
  const refresh = async () => {
    await Promise.all([
      qc.invalidateQueries({ queryKey: trpc.photos.list.queryKey({ projectId }) }),
  ...
  async function uploadFiles(files: File[]) {
    ...
    for (const [i, file] of files.entries()) {
      try {
        ...
        await client.photos.completeUpload.mutate({ projectId, uploadId: begin.uploadId });
        ok++;
      } catch (e) {
        toast("error", e instanceof PhotoError ? `${file.name}: ${e.message}` : `${file.name}: ${errorMessage(e)}`);
      }
  ...
  const setHero = useMutation(
    trpc.photos.setHero.mutationOptions({
      onSuccess: async (_d, v) => {
        toast("success", v.photoId ? "Pinned as the hero photo" : "The newest photo is the hero again");
        await refresh();
      },
  ...
  return (
    <section aria-labelledby="photos-h">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        {canUpload && (
          <>
            ...
            />
            <Button variant="secondary" onClick={() => input.current?.click()} loading={!!progress}>
              {progress ? `Uploading ${progress.done + 1} of ${progress.total}…` : (
                <>
      ...
      {photos.isPending ? (
        ...
        <div className="rounded-card border border-dashed border-border-strong bg-surface/60 px-6 py-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm text-muted">
            {canUpload ? "Add site photos from your phone or computer. They're compressed before upload; the newest becomes the hero unless you pin one." : "Photos added by the team will appear here."}
          </p>
        </div>
      ) : (
        <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {list.map((p) => (
            <li key={p.id} className="relative">
              ...
              {p.id === heroPhotoId && (
                <StatusPill tone="accent" className="pointer-events-none absolute left-2 top-2">
            ...
            <>
              ...
              {canManage &&
                (pinnedHeroId === current.id ? (
                  <Button variant="secondary" loading={setHero.isPending} onClick={() => setHero.mutate({ projectId, photoId: null })}>
                    Unpin hero
                  </Button>
                ) : (
                  <Button variant="secondary" loading={setHero.isPending} onClick={() => setHero.mutate({ projectId, photoId: current.id })}>
                    Pin as hero
                  </Button>
                ))}
              <a href={photoUrl(current.id, "full")} target="_blank" rel="noopener" className="inline-flex h-10 items-center rounded-control px-4 text-sm font-medium hover:bg-sunken">
                Full size
              </a>
            </>
          )
      ...
      >
        {current && (
          // eslint-disable-next-line @next/next/no-img-element -- private, access-checked route
          <img src={photoUrl(current.id, "full")} alt="" className={cn("max-h-[65vh] w-full rounded-panel object-contain")} />
        )}
      </Dialog>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => removing && remove.mutate({ projectId, photoId: removing })}
      />
    </section>
  );
}
