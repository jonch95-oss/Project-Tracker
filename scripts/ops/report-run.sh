...
if [ -z "${APP_URL:-}" ] || [ -z "${JOB_SECRET:-}" ]; then echo "Reporting skipped (secrets not set)"; exit 0; fi
now=$(date +%s)
...
curl -sS --max-time 30 -X POST -H "Authorization: Bearer ${JOB_SECRET}" -H "content-type: application/json" \
