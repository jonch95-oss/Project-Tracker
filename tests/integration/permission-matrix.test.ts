...
const ACTIVE: Scenario[] = SCENARIOS.filter(
  (s) => s !== "anon" && s !== "deactivated",
);
...
const INTERNAL_ASSIGNED: Scenario[] = [
  "owner",
  "admin+fin",
  "admin",
  "member+fin",
...
const TEMPLATE_EDITORS: Scenario[] = [
  "owner",
  "admin+fin",
  "admin",
  "admin-unassigned",
...
async function vendorFixture(): Promise<string> {
  const [v] = await db()
    ...
    .values({ name: `Matrix Vendor ${uid()}`, key: `matrix vendor ${uid()}` })
...
async function unitFixture(
  projectId: string,
): Promise<{ unitId: string; selectionId: string; version: number }> {
  const [u] = await db()
    .insert(schema.saleUnit)
    .values({ projectId, unit: `M${uid()}` })
    .returning({ id: schema.saleUnit.id });
  const [sel] = await db()
    .insert(schema.unitSelection)
    .values({ projectId, unitId: u!.id, category: "Kitchen", choice: "Matrix" })
    .returning();
...
async function investorFixture(projectId: string): Promise<string> {
  const [inv] = await db()
    .insert(schema.investor)
    .values({ name: `Matrix Investor ${uid()}` })
    .returning({ id: schema.investor.id });
  await db()
    .insert(schema.capitalCommitment)
    .values({ projectId, investorId: inv!.id, committedCents: 100_000_00 });
  return inv!.id;
}

async function taskVersion(id: string): Promise<number> {
  const [t] = await db()
    .select({ version: schema.task.version })
    .from(schema.task)
...
async function folderId(projectId: string, name = "Design"): Promise<string> {
  const [folder] = await db()
    ...
    .from(schema.folder)
    .where(
      and(eq(schema.folder.projectId, projectId), eq(schema.folder.name, name)),
    );
  return folder!.id;
}
...
>(
  ...
  const [r] = await db()
    .insert(t)
    .values({ projectId, ...base[table], ...values } as never)
    .returning();
  return {
...
async function freshTask(
  projectId: string,
  extra: Partial<typeof schema.task.$inferInsert> = {},
): Promise<string> {
  const [t] = await db()
    .insert(schema.task)
    .values({
...
const MATRIX: Record<string, Row | "public"> = {
  ...
  "users.setRole": {
    allowed: OWNER_ONLY,
    call: (c, f) => c.users.setRole({ userId: f.otherUserId, role: "member" }),
  },
  "users.setStatus": {
    allowed: OWNER_ONLY,
    call: (c, f) =>
      c.users.setStatus({ userId: f.otherUserId, status: "active" }),
  },
  "users.createResetLink": {
    allowed: OWNER_ONLY,
    call: (c, f) => c.users.createResetLink({ userId: f.otherUserId }),
  },
  "users.access": {
    allowed: OWNER_ONLY,
    call: (c, f) => c.users.access({ userId: f.otherUserId }),
  },
  ...
  "photos.completeUpload": {
    ...
    call: async (c, f) => {
      ...
      for (const o of b.objects)
        await storage().put(
          o.pathname,
          new Uint8Array(o.role === "full" ? 10 : 5),
          { contentType: "image/webp" },
        );
      return c.photos.completeUpload({
        projectId: f.projectId,
        uploadId: b.uploadId,
      });
    },
  ...
  "checklist.setDone": {
    ...
    call: async (c, f) =>
      c.checklist.setDone({
        ...
        version: await taskVersion(f.taskId),
      }),
  },
  "checklist.addTask": {
    allowed: EDITORS,
  ...
  "checklist.deleteTask": {
    ...
    call: async (c, f) => {
      const [t] = await db()
        .insert(schema.task)
        .values({ projectId: f.projectId, phaseKey: "pipeline", title: "Temp" })
        .returning({ id: schema.task.id });
    ...
    call: async (c, f) => {
      const ids = (
        await db()
          ...
          .where(
            and(
              eq(schema.task.projectId, f.projectId),
              eq(schema.task.phaseKey, "closing"),
            ),
          )
      ).map((r) => r.id);
      return c.checklist.reorder({
        projectId: f.projectId,
        phaseKey: "closing",
        taskIds: ids,
      });
  ...
  "checklist.saveAsTemplate": {
    ...
    call: (c, f) =>
      c.checklist.saveAsTemplate({
        projectId: f.projectId,
        name: `From matrix ${uid()}`,
      }),
  },

  "templates.list": {
    allowed: TEMPLATE_EDITORS,
    call: (c) => c.templates.list(),
  },
  "templates.get": {
    allowed: TEMPLATE_EDITORS,
    call: (c, f) => c.templates.get({ templateId: f.templateId }),
  },
  "templates.create": {
    allowed: TEMPLATE_EDITORS,
    call: (c) =>
  ...
  "templates.save": {
    ...
    call: async (c, f) => {
      const t = await c.templates
        .get({ templateId: f.templateId })
        .catch(() => null);
      const [row] = await db()
        .select()
        .from(schema.template)
        .where(eq(schema.template.id, f.templateId));
      return c.templates.save({
        templateId: f.templateId,
        version: t?.version ?? row!.version,
        definition: (t?.definition ?? row!.definition) as never,
      });
    },
  },
  "templates.setDefault": {
    allowed: TEMPLATE_EDITORS,
    call: (c, f) => c.templates.setDefault({ templateId: f.templateId }),
  },
  ...
  "templates.applyUpdate": {
    ...
    call: async (c, f) => {
      const [t] = await db()
        ...
        .from(schema.template)
        .where(eq(schema.template.id, f.templateId));
      return c.templates.applyUpdate({
  ...
  "checklist.applyTemplateUpdate": {
    ...
    call: async (c, f) => {
      const [t] = await db()
        ...
        .from(schema.template)
        .where(eq(schema.template.id, f.templateId));
      return c.checklist.applyTemplateUpdate({
        projectId: f.projectId,
        expectedVersion: t!.version,
      });
    },
    ...
    call: async (c, f) => {
      const [t] = await db()
        ...
        .where(
          and(
            eq(schema.task.projectId, f.projectId),
            eq(schema.task.templateKey, "dot_permits"),
          ),
        );
      return c.checklist.setSubItem({
        projectId: f.projectId,
        taskId: t!.id,
        itemId: t!.subItems[0]!.id,
        done: false,
      });
    },
  },
  ...
  "tasks.setStatus": {
    ...
    call: async (c, f) =>
      c.tasks.setStatus({
        projectId: f.projectId,
        taskId: await freshTask(f.projectId),
        version: 1,
        status: "in_progress",
      }),
  },
  ...
  "tasks.bulk": {
    ...
    call: async (c, f) =>
      c.tasks.bulk({
        projectId: f.projectId,
        taskIds: [await freshTask(f.projectId)],
        action: { kind: "redate", dueOn: "2030-01-15" },
      }),
  },
  "tasks.shiftPhase": {
    allowed: EDITORS,
  ...
  "keyDates.remove": {
    ...
    call: async (c, f) => {
      const [d] = await db()
        ...
        .values({ projectId: f.projectId, kind: "auction", date: "2030-03-01" })
        .returning();
  ...
  "notifications.unreadCount": {
    allowed: ACTIVE,
    call: (c) => c.notifications.unreadCount(),
  },
  ...
  "records.syncNow": {
    ...
    call: (c, f) =>
      c.records
        .syncNow({ projectId: f.projectId })
        .catch((e: { code?: string }) => {
          if (e.code !== "BAD_REQUEST" && e.code !== "TOO_MANY_REQUESTS")
            throw e;
        }),
  },
  ...
  "records.updateViolation": {
    ...
    call: async (c, f) => {
      const [v] = await db()
        ...
        .returning();
      return c.records.updateViolation({
        projectId: f.projectId,
        id: v!.id,
        version: 1,
        stage: "fixed",
  ...
  "siteLogs.save": {
    ...
    call: (c, f) =>
      c.siteLogs
        .save({
          projectId: f.projectId,
          date: `20${10 + Math.floor(Math.random() * 15)}-0${1 + Math.floor(Math.random() * 9)}-${10 + Math.floor(Math.random() * 18)}`,
          manpower: [],
          inspections: [],
          delays: [],
          weather: {
  ...
  "schedule.requestRebaseline": {
    ...
    call: async (c, f) => {
      await db()
        ...
        .where(
          and(
            eq(schema.scheduleBaseline.projectId, f.projectId),
            eq(schema.scheduleBaseline.status, "requested"),
          ),
        );
      return c.schedule
        .requestRebaseline({ projectId: f.projectId, reason: "Matrix check" })
        .catch((e: { code?: string }) => {
          if (e.code !== "BAD_REQUEST") throw e;
        });
    },
  ...
  "schedule.decideRebaseline": {
    ...
    call: async (c, f) => {
      ...
      const [r] = await db()
        ...
        .values({
          projectId: f.projectId,
          number: 100_000 + Math.floor(Math.random() * 900_000),
          status: "requested",
          reason: "Matrix",
        })
        .returning();
      return c.schedule.decideRebaseline({
        projectId: f.projectId,
        id: r!.id,
        approve: false,
      });
  ...
  "meetings.deleteItem": {
    ...
    call: async (c, f) => {
      const [m] = await db()
        ...
        .returning();
      const [i] = await db()
        .insert(schema.meetingItem)
        .values({ meetingId: m!.id, kind: "note", text: "x" })
        .returning();
  ...
  "rfis.attach": {
    ...
    call: async (c, f) => {
      const [r] = await db()
        ...
        .returning();
      return c.rfis.attach({
        projectId: f.projectId,
        id: r!.id,
        fileId: await freshFile(f.projectId),
        on: true,
      });
    },
  },
  ...
  "submittals.decide": {
    ...
    call: async (c, f, me) => {
      const [s] = await db()
        ...
        .returning();
      await db()
        .insert(schema.submittalRevision)
        .values({ submittalId: s!.id, revision: 0 });
      return c.submittals.decide({
        projectId: f.projectId,
        id: s!.id,
        version: 1,
        decision: "approved",
  ...
  "submittals.resubmit": {
    ...
    call: async (c, f) => {
      ...
      await db()
        .insert(schema.submittalRevision)
        .values({ submittalId: s!.id, revision: 0, decision: "rejected" });
      return c.submittals.resubmit({
        projectId: f.projectId,
        id: s!.id,
        version: 1,
        fileId: null,
  ...
  "drawings.createSet": {
    ...
    call: async (c, f) =>
      c.drawings.createSet({
        ...
        name: "Matrix",
        issuedOn: null,
        fileIds: [await freshFile(f.projectId)],
      }),
  },
  "drawings.sheet": {
    allowed: INTERNAL_ASSIGNED,
    call: async (c, f) => {
      const [set] = await db()
        ...
        .values({ projectId: f.projectId, discipline: "E", name: "Matrix" })
        .returning();
      const fileId = await freshFile(f.projectId);
      const [v] = await db()
        .select({ id: schema.fileVersion.id })
        .from(schema.fileVersion)
        .where(eq(schema.fileVersion.fileId, fileId));
      const [sh] = await db()
        .insert(schema.drawingSheet)
        .values({
          setId: set!.id,
          projectId: f.projectId,
          number: "E-1",
  ...
  "notifySettings.get": {
    allowed: ACTIVE,
    call: (c) => c.notifySettings.get(),
  },
  "notifySettings.save": {
    allowed: ACTIVE,
    call: (c) =>
      c.notifySettings.save({
        prefs: {},
        quietStart: null,
        quietEnd: null,
        digest: true,
  ...
  "push.unsubscribe": {
    ...
    call: (c) =>
      c.push.unsubscribe({
        endpoint: "https://fcm.googleapis.com/fcm/send/none",
      }),
  },
  "push.test": {
    allowed: ACTIVE,
    call: (c) =>
      c.push.test().catch((e: { code?: string }) => {
  ...
  "push.removeDevice": {
    allowed: ACTIVE,
    call: (c) =>
      c.push.removeDevice({ id: "00000000-0000-4000-8000-000000000000" }),
  },
  ...
  "files.list": {
    ...
    call: async (c, f) =>
      c.files.list({
        projectId: f.projectId,
        folderId: await folderId(f.projectId),
      }),
  },
  "files.canRead": {
    allowed: INTERNAL_ASSIGNED,
  ...
  "files.completeUpload": {
    ...
    call: async (c, f) => {
      ...
      for (const o of b.objects)
        await storage().put(o.pathname, new Uint8Array(10), {
          contentType: o.contentType,
        });
      return c.files.completeUpload({
        projectId: f.projectId,
        uploadId: b.uploadId,
      });
    },
  ...
  "files.move": {
    ...
    call: async (c, f) =>
      c.files.move({
        ...
        fileId: await freshFile(f.projectId),
        version: 1,
        folderId: await folderId(f.projectId, "Legal"),
      }),
  },
  "files.remove": {
    allowed: EDITORS,
  ...
  "files.restore": {
    ...
    call: async (c, f) =>
      c.files.restore({
        projectId: f.projectId,
        fileId: await freshFile(f.projectId, { trashed: true }),
      }),
  },
  "files.purge": {
    allowed: EDITORS,
  ...
  "files.renameFolder": {
    ...
    call: async (c, f) => {
      const [x] = await db()
        ...
        .values({ projectId: f.projectId, name: `R ${uid()}` })
        .returning();
      return c.files.renameFolder({
        projectId: f.projectId,
        folderId: x!.id,
        name: `R2 ${uid()}`,
      });
  ...
  "files.deleteFolder": {
    ...
    call: async (c, f) => {
      const [x] = await db()
        .insert(schema.folder)
        .values({ projectId: f.projectId, name: `D ${uid()}` })
        .returning();
  ...
  "financials.overview": {
    allowed: FIN_VIEW,
    call: (c, f) => c.financials.overview({ projectId: f.projectId }),
  },
  "financials.saveHeadline": {
    allowed: FIN_EDIT,
    call: async (c, f) =>
      c.financials.saveHeadline({
        projectId: f.projectId,
        version:
          (
            await db()
              .select()
              .from(schema.projectHeadline)
              .where(eq(schema.projectHeadline.projectId, f.projectId))
          )[0]?.version ?? 0,
        purchasePriceCents: 1,
        totalBudgetCents: null,
        projectedSelloutCents: null,
        loanAmountCents: null,
        useBudgetDetail: false,
  ...
  "financials.deleteLine": {
    allowed: FIN_EDIT,
    call: async (c, f) =>
      c.financials.deleteLine({
        projectId: f.projectId,
        ...(await fin("budgetLine", f.projectId)),
      }),
  },
  "financials.startBudget": {
    allowed: FIN_EDIT,
    call: (c, f) => c.financials.startBudget({ projectId: f.projectId }),
  },
  "financials.saveCommitment": {
    allowed: FIN_EDIT,
    call: (c, f) =>
      c.financials.saveCommitment({
        projectId: f.projectId,
        vendorName: "V",
        amountCents: 100,
      }),
  },
  "financials.deleteCommitment": {
    allowed: FIN_EDIT,
    call: async (c, f) =>
      c.financials.deleteCommitment({
        projectId: f.projectId,
        ...(await fin("commitment", f.projectId)),
      }),
  },
  "financials.saveInvoice": {
    allowed: FIN_EDIT,
    call: (c, f) =>
      c.financials.saveInvoice({
        projectId: f.projectId,
        vendorName: "V",
        amountCents: 100,
      }),
  ...
  "financials.decideInvoice": {
    ...
    call: async (c, f) => {
      ...
      return c.financials.decideInvoice({
        projectId: f.projectId,
        id: inv.id,
        version: inv.version,
        decision: "approved",
  ...
  "financials.reopenInvoice": {
    ...
    call: async (c, f) => {
      const inv = await fin("invoice", f.projectId, { status: "approved" });
      return c.financials.reopenInvoice({
        projectId: f.projectId,
        id: inv.id,
        version: inv.version,
        note: "Wrong amount",
  ...
  "financials.deleteInvoice": {
    allowed: FIN_EDIT,
    call: async (c, f) =>
      c.financials.deleteInvoice({
        projectId: f.projectId,
        ...(await fin("invoice", f.projectId)),
      }),
  },
  "financials.saveChangeOrder": {
    allowed: FIN_EDIT,
    call: (c, f) =>
      c.financials.saveChangeOrder({
        projectId: f.projectId,
        description: "x",
        amountCents: 100,
      }),
  ...
  "financials.decideChangeOrder": {
    ...
    call: async (c, f) => {
      const line = await fin("budgetLine", f.projectId);
      const co = await fin("changeOrder", f.projectId, {
        budgetLineId: line.id,
      });
      return c.financials.decideChangeOrder({
        projectId: f.projectId,
        id: co.id,
        version: co.version,
        decision: "approved",
  ...
  "financials.deleteChangeOrder": {
    allowed: FIN_EDIT,
    call: async (c, f) =>
      c.financials.deleteChangeOrder({
        projectId: f.projectId,
        ...(await fin("changeOrder", f.projectId)),
      }),
  },
  "financials.createDraw": {
    ...
    call: async (c, f) => {
      await db()
        ...
        .where(
          and(
            eq(schema.draw.projectId, f.projectId),
  ...
  "financials.updateDraw": {
    allowed: FIN_EDIT,
    call: async (c, f) => {
      const d = await fin("draw", f.projectId, { status: "submitted" });
      return c.financials.updateDraw({
        projectId: f.projectId,
        id: d.id,
        version: d.version,
        notes: "n",
  ...
  "financials.advanceDraw": {
    allowed: FIN_EDIT,
    call: async (c, f) => {
      const d = await fin("draw", f.projectId, {
        status: "inspector_approved",
      });
      return c.financials.advanceDraw({
        projectId: f.projectId,
        id: d.id,
        version: d.version,
      });
  ...
  "financials.deleteDraw": {
    allowed: FIN_EDIT,
    call: async (c, f) =>
      c.financials.deleteDraw({
        projectId: f.projectId,
        ...(await fin("draw", f.projectId, { status: "draft" })),
      }),
  },
  "financials.saveUnit": {
    allowed: FIN_EDIT,
    call: (c, f) =>
      c.financials.saveUnit({
        projectId: f.projectId,
        unit: `U${uid()}`,
        status: "available",
      }),
  },
  "financials.deleteUnit": {
    allowed: FIN_EDIT,
    call: async (c, f) =>
      c.financials.deleteUnit({
        projectId: f.projectId,
        ...(await fin("saleUnit", f.projectId)),
      }),
  },
  ...
  "projects.lookupLot": {
    allowed: TEMPLATE_EDITORS,
    call: (c) =>
      c.projects.lookupLot({ bbl: "3017590013", borough: "Brooklyn" }),
  },
  ...
  "units.saveSelection": {
    ...
    call: async (c, f) =>
      c.units.saveSelection({
        projectId: f.projectId,
        unitId: (await unitFixture(f.projectId)).unitId,
        category: "Bath",
        choice: "Matrix tile",
      }),
  },
  ...
  "directory.options": {
    allowed: DIR_VIEW,
    call: (c) => c.directory.options(),
  },
  "directory.get": {
    allowed: DIR_VIEW,
    call: async (c) => c.directory.get({ id: await vendorFixture() }),
  },
  "directory.saveVendor": {
    allowed: DIR_EDIT,
    call: (c) =>
      c.directory.saveVendor({
        name: `Matrix Co ${uid()}`,
        kind: "contractor",
      }),
  },
  "directory.setArchived": {
    allowed: DIR_EDIT,
    call: async (c) =>
      c.directory.setArchived({ id: await vendorFixture(), archived: true }),
  },
  "directory.saveContact": {
    allowed: DIR_EDIT,
    call: async (c) =>
      c.directory.saveContact({
        vendorId: await vendorFixture(),
        name: "Matrix Person",
      }),
  },
  ...
  "directory.beginDocUpload": {
    ...
    call: async (c) => {
      const [d] = await db()
        .insert(schema.vendorDocument)
        .values({ vendorId: await vendorFixture(), kind: "w9", category: "w9" })
        .returning({ id: schema.vendorDocument.id });
      return c.directory.beginDocUpload({
        documentId: d!.id,
        name: "w9.pdf",
        contentType: "application/pdf",
        sizeBytes: 10,
  ...
  "directory.completeDocUpload": {
    ...
    call: async (c, f, me) => {
      ...
      const b = await c.directory.beginDocUpload({
        documentId: d!.id,
        name: "w9.pdf",
        contentType: "application/pdf",
        sizeBytes: 10,
  ...
  "capital.overview": {
    allowed: FIN_VIEW,
    call: (c, f) => c.capital.overview({ projectId: f.projectId }),
  },
  "capital.saveTerms": {
    allowed: FIN_EDIT,
    call: async (c, f) => {
      const [t] = await db()
        .select({ version: schema.capitalTerms.version })
        .from(schema.capitalTerms)
        .where(eq(schema.capitalTerms.projectId, f.projectId));
      return c.capital.saveTerms({
        projectId: f.projectId,
        version: t?.version ?? 0,
        tiers: [
          { kind: "return_of_capital" },
          { kind: "split", lpBps: 8000, untilMultipleMilli: null },
        ],
      });
    },
  },
  "capital.saveInvestor": {
    allowed: FIN_EDIT,
    call: (c, f) =>
      c.capital.saveInvestor({
        projectId: f.projectId,
        name: `Matrix LP ${uid()}`,
        kind: "equity",
        committedCents: 50_000_00,
      }),
  },
  "capital.setCommitment": {
    ...
    call: async (c, f) => {
      const [inv] = await db()
        .insert(schema.investor)
        .values({ name: `Matrix Attach ${uid()}` })
        .returning({ id: schema.investor.id });
      return c.capital.setCommitment({
        projectId: f.projectId,
        investorId: inv!.id,
        committedCents: 1_000_00,
      });
    },
  },
  "capital.removeInvestor": {
    allowed: FIN_EDIT,
    call: async (c, f) =>
      c.capital.removeInvestor({
        projectId: f.projectId,
        investorId: await investorFixture(f.projectId),
      }),
  },
  "capital.createCall": {
    allowed: FIN_EDIT,
  ...
  "capital.recordReceipt": {
    ...
    call: async (c, f) => {
      ...
      const [call] = await db()
        ...
        .returning();
      const [item] = await db()
        .insert(schema.capitalCallItem)
        .values({ callId: call!.id, investorId: inv, amountCents: 100 })
        .returning();
      return c.capital.recordReceipt({
        projectId: f.projectId,
        itemId: item!.id,
  ...
  "capital.previewDistribution": {
    ...
    call: async (c, f) => {
      await investorFixture(f.projectId);
      return c.capital.previewDistribution({
        projectId: f.projectId,
  ...
  "capital.recordDistribution": {
    ...
    call: async (c, f) => {
      await investorFixture(f.projectId);
      return c.capital.recordDistribution({
        projectId: f.projectId,
        amountCents: 1_000_00,
        paidOn: todayET(),
      });
  ...
  "capital.deleteDistribution": {
    ...
    call: async (c, f) => {
      const [latest] = await db()
        .select({ n: schema.distribution.number })
        .from(schema.distribution)
        .where(eq(schema.distribution.projectId, f.projectId))
        .orderBy(desc(schema.distribution.number))
        .limit(1);
      const [d] = await db()
        .insert(schema.distribution)
        .values({
          projectId: f.projectId,
          number: (latest?.n ?? 0) + 1,
          paidOn: todayET(),
        ...
        .returning();
      return c.capital.deleteDistribution({
        projectId: f.projectId,
        id: d!.id,
      });
    },
  ...
  "portal.list": {
    allowed: ACTIVE.filter((s) => !s.startsWith("external")),
  ...
  "calendar.create": {
    allowed: ACTIVE.filter((s) => !s.startsWith("investor")),
    call: (c) => c.calendar.create(),
  },
  "calendar.revoke": { allowed: ACTIVE, call: (c) => c.calendar.revoke() },

  "analytics.overview": {
    allowed: OWNER_ONLY,
    call: (c) => c.analytics.overview(),
  },
  "analytics.durationSuggestions": {
    allowed: OWNER_ONLY,
    call: (c) => c.analytics.durationSuggestions(),
  },
  "analytics.applyDurations": {
    ...
    call: async (c) => {
      ...
      ).tasks.find((x) => x.due?.from === "phase_start")!;
      return c.analytics.applyDurations({
        templateId: t!.id,
        version: t!.version,
        changes: [{ templateKey: k.key, days: k.due!.days }],
      });
    },
  },
...
describe("permission matrix", () => {
  ...
  beforeAll(async () => {
    ...
    const [firstTask] = await db()
      ...
      .where(
        and(
          ...
          eq(schema.task.templateKey, "mih_check"),
        ),
      );
    fixture.taskId = firstTask!.id;
    const [proj] = await db()
      .select({ templateId: schema.project.templateId })
      .from(schema.project)
    ...
    fixture.photoId = async () => {
      const b = await ownerCaller.photos.beginUpload({
        projectId: p.id,
        contentType: "image/webp",
        fullBytes: 10,
        thumbBytes: 5,
      ...
      return (
        await ownerCaller.photos.completeUpload({
          projectId: p.id,
          uploadId: b.uploadId,
        })
      ).id;
      ...
      for (const scenario of SCENARIOS) {
        ...
        it(`${scenario}: ${expectAllowed ? "allowed" : "denied"}`, async () => {
          ...
          if (expectAllowed) {
            expect(
              error,
              `${path} should succeed for ${scenario}: ${String(error)}`,
            ).toBeNull();
          } else {
            expect(error).toBeInstanceOf(TRPCError);
            expect(
              DENIED_CODES.has((error as TRPCError).code),
              `unexpected code ${(error as TRPCError).code}`,
            ).toBe(true);
          }
        });
      }
  ...
  it("unassigned users get NOT_FOUND, so project existence does not leak", async () => {
    for (const s of [
      ...
      await expect(
        c.projects.get({ projectId: fixture.projectId }),
  ...
  it("externals do not see other members' emails or permission flags", async () => {
    ...
    const asMember = await (
      await callerFor(users.member)
    ).members.list({ projectId: fixture.projectId });
  ...
  it("admins cannot grant financial visibility they do not have", async () => {
    ...
    const [row] = await db()
      ...
      .where(eq(schema.projectMember.userId, target.id));
