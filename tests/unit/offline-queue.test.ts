...
const trpcError = (code: string, message = code) => Object.assign(new Error(message), { data: { code } });
...
describe("offline queue", () => {
  it("saves ticks and comments with readable labels; a tick and untick cancel out", () => {
    q.enqueue("checklist.setDone", { projectId: P, taskId: T1, done: true, version: 3 });
    q.enqueue("tasks.addComment", { projectId: P, taskId: T2, body: "On site" });
    expect(q.readQueue().map((o) => o.label)).toEqual(["Tick “Order survey”", "Comment"]);
    q.enqueue("checklist.setDone", { projectId: P, taskId: T1, done: false, version: 3 });
  ...
  it("answers queued calls at once", () => {
    expect(q.queuedResult("checklist.setDone", { projectId: P, taskId: T1, done: true, version: 4 })).toEqual({ status: "done", version: 4, queued: true });
    expect(q.queuedResult("checklist.setDone", { projectId: P, taskId: T1, done: false, version: 4 })).toMatchObject({ status: "not_started" });
    expect(q.queuedResult("tasks.addComment", { projectId: P, taskId: T1, body: "x" })).toMatchObject({ queued: true });
  ...
  it("shows queued changes on top of fresh data", () => {
    q.enqueue("checklist.setDone", { projectId: P, taskId: T1, done: true, version: 1 });
    q.enqueue("tasks.addComment", { projectId: P, taskId: T1, body: "Queued note" });
    const cl = q.overlayQueued("checklist.get", { projectId: P }, { tasks: [{ id: T1, status: "not_started" }, { id: T2, status: "in_progress" }] }) as { tasks: { id: string; status: string }[] };
    expect(cl.tasks.map((t) => t.status)).toEqual(["done", "in_progress"]);
    const mine = q.overlayQueued("tasks.mine", undefined, { total: 2, sections: [{ groups: [{ tasks: [{ id: T1 }, { id: T2 }] }, { tasks: [{ id: T1 }] }] }] }) as { total: number; sections: { groups: { tasks: { id: string }[] }[] }[] };
    expect(mine.total).toBe(0);
    expect(mine.sections[0]!.groups).toEqual([{ tasks: [{ id: T2 }] }]);
    const detail = q.overlayQueued("tasks.detail", { projectId: P, taskId: T1 }, { status: "not_started", comments: [{ id: "c1" }] }) as { status: string; comments: { id: string; body?: string; authorName?: string }[] };
    expect(detail.status).toBe("done");
    expect(detail.comments).toHaveLength(2);
    expect(detail.comments[1]).toMatchObject({ body: "Queued note", authorName: "You · waiting to sync" });
    // Other queries, other projects and empty data pass through.
    const other = { tasks: [{ id: T1, status: "not_started" }] };
    expect(q.overlayQueued("projects.get", { projectId: P }, other)).toBe(other);
    expect(q.overlayQueued("checklist.get", { projectId: "someone-else" }, other)).toBe(other);
    expect(q.overlayQueued("checklist.get", { projectId: P }, null)).toBeNull();
    expect(q.overlayQueued("checklist.get", { projectId: P }, { nope: 1 })).toEqual({ nope: 1 });
    // Another person's queue never shows.
    q.setQueueViewer("user-2");
    expect(q.overlayQueued("checklist.get", { projectId: P }, other)).toBe(other);
  });

  it("an untick shows as not started, and a queued untick on My Tasks leaves the task there", () => {
    q.enqueue("checklist.setDone", { projectId: P, taskId: T1, done: false, version: 5 });
    const cl = q.overlayQueued("checklist.get", { projectId: P }, { tasks: [{ id: T1, status: "done" }] }) as { tasks: { status: string }[] };
    expect(cl.tasks[0]!.status).toBe("not_started");
    const mine = q.overlayQueued("tasks.mine", undefined, { sections: [{ groups: [{ tasks: [{ id: T1 }] }] }] }) as { sections: { groups: unknown[] }[] };
  ...
  it("sends in order, carries new versions forward, holds conflicts and failures, and stops when the connection drops", async () => {
    ...
    q.enqueue("checklist.setDone", { projectId: P, taskId: T2, done: true, version: 7 });
    q.enqueue("tasks.addComment", { projectId: P, taskId: T2, body: "refused" });
    const calls: string[] = [];
    ...
    expect(calls).toEqual(["done T1 v1", "comment first", "done T2 v7", "comment refused"]);
    ...
    const base = { userId: "user-1", queuedAt: new Date().toISOString(), label: "x", state: "pending" as const, path: "checklist.setDone" as const };
    store.set("pc.offline.queue.v1", JSON.stringify([{ ...base, id: "a", input: { projectId: P, taskId: T1, done: true, version: 2 } }, { ...base, id: "b", input: { projectId: P, taskId: T1, done: false, version: 2 } }]));
    const seen: number[] = [];
    await q.flushQueue({ ...client, setDone: async (i) => (seen.push(i.version), { status: "done", version: i.version + 1 }) }, String);
    expect(seen).toEqual([2, 3]);

    // No connection: nothing is lost.
    q.enqueue("tasks.addComment", { projectId: P, taskId: T1, body: "later" });
    const offline = await q.flushQueue({ ...client, addComment: async () => Promise.reject(new TypeError("Failed to fetch")) }, String);
    expect(offline).toEqual({ sent: 0, left: 1 });
    expect(q.readQueue()[0]!.state).toBe("pending");
  });

  it("drops what someone else queued on this device and resolves only ticks", async () => {
    q.setQueueViewer("user-9");
    q.enqueue("tasks.addComment", { projectId: P, taskId: T1, body: "theirs" });
    q.setQueueViewer("user-1");
    const client = { setDone: async () => ({ status: "done", version: 1 }), addComment: async () => ({}), latestVersion: async () => null };
    ...
    const c = q.enqueue("tasks.addComment", { projectId: P, taskId: T1, body: "x" });
    await q.resolveConflict(client, c.id, String);
    expect(q.readQueue()).toHaveLength(1);
    // A task that's gone.
    q.clearQueue();
    const t = q.enqueue("checklist.setDone", { projectId: P, taskId: T1, done: true, version: 1 });
    await q.resolveConflict(client, t.id, String);
    expect(q.readQueue()[0]).toMatchObject({ state: "failed", message: "This task is no longer on the project." });
  });

  it("tells a dropped connection from a refusal", () => {
    expect(q.isNetworkError(new TypeError("Failed to fetch"))).toBe(true);
    expect(q.isNetworkError(Object.assign(new Error("x"), { cause: new TypeError("Load failed") }))).toBe(true);
    expect(q.isNetworkError(new Error("NetworkError when attempting to fetch resource."))).toBe(true);
